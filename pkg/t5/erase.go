...
func (t *Client) EraseFlash(ctx context.Context) error {
	if !t.bootloaded {
	...
	frame := model.NewFrame(0x5, cmd, model.ResponseRequired)
	resp, err := t.c.SendAndPoll(ctx, frame, 10*time.Second, 0xC)
	if err != nil {
		return err
	}
	data := resp.Data()
