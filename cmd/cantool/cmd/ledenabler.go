...
var ledenablerCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		f, err := gm.ReadDataByIdentifier(ctx, 0x249, 0x45) // Read REC Bulb Outage &Substitution Lighting
		if err != nil {
			return err
		}
		log.Println("BO REC", string(f[:]))

		f0, err := gm.ReadDataByIdentifier(ctx, 0x24F, 0x40) // Read PWM Settings for Bulbs
		if err != nil {
			return err
		}
		log.Println("PWM UEC", string(f0[:]))

		f2, err := gm.ReadDataByIdentifier(ctx, 0x24F, 0x4D) // Read UHEC Bulb Outage & Substitution
		if err != nil {
			return err
		}
		...
		if result == "Yes" {
			err := gm.WriteDataByIdentifier(ctx, 0x249, 0x45, []byte{0x00}) // set Bulb Outage & Substitution Lighting to 0?
			if err != nil {
				return errors.New("failed to disable bulb outage in REC ")
			}
			err = gm.WriteDataByIdentifier(ctx, 0x24F, 0x4F, []byte{0x00}) // set Bulb Outage & Substitution to 0?
			if err != nil {
