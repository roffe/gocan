...
func (stn *STNFTDI) Open(ctx context.Context) error {
	...
	if err := stn.port.SetLatency(2); err != nil {
		stn.port.Close()
		return err
	}
	...
	for _, from := range [...]uint{115200, 38400, 230400, 921600, 2000000, 1000000, 57600} {
		...
		if stn.trySpeed(stn.port, from, to) == nil {
			found = true
			break
		}
...
func (stn *STNFTDI) trySpeed(p *ftdi.Device, from, to uint) error {
	if err := p.SetBaudRate(uint(from)); err != nil {
		return err
	}

	if _, err := p.Write([]byte("\r\r\r")); err != nil {
		return err
	}

	time.Sleep(20 * time.Millisecond)

	if _, err := p.Write([]byte("STBR" + strconv.Itoa(int(to)) + "\r")); err != nil {
		return err
	}
	time.Sleep(10 * time.Millisecond)
	if err := p.Purge(ftdi.FT_PURGE_RX); err != nil {
		return err
	}
	if err := p.SetBaudRate(uint(to)); err != nil {
		return err
	}
	...
	for range 10 {
		...
		if bytes.Contains(readbuff[:n], []byte("STN")) {
